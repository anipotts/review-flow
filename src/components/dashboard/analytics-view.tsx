  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div>
          <label htmlFor="filter-from" className="block text-sm font-medium text-ink-secondary mb-1.5">
            From
          </label>
          <input
            id="filter-from"
            ...
            className="w-full px-3 py-2.5 bg-surface border border-edge rounded-lg text-sm text-ink focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent min-h-[44px]"
          />
        </div>
        <div>
          <label htmlFor="filter-to" className="block text-sm font-medium text-ink-secondary mb-1.5">
            To
          </label>
          <input
            id="filter-to"
                ...
                {[5, 4, 3, 2, 1].map((rating) => {
                  ...
                  return (
                    <div key={rating} className="flex items-center gap-2 sm:gap-3">
                      <span className="text-xs sm:text-sm font-medium text-ink-secondary w-8 sm:w-12 shrink-0">
                        {rating}&#9733;
                      </span>
                      <div className="flex-1 bg-surface-hover rounded-full h-3 sm:h-4 overflow-hidden">
                        <div
                      ...
                      <span className="text-xs sm:text-sm text-ink-muted w-6 sm:w-8 text-right shrink-0">
                        {count}
                      </span>
                    </div>
                  );
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-edge-subtle">
                    ...
                    <th className="text-left text-xs font-medium text-ink-muted px-6 py-3">Redirected To</th>
                    <th className="text-left text-xs font-medium text-ink-muted px-6 py-3">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {data.clicks.map((click) => (
                    <tr key={click.id} className="border-b border-edge-subtle hover:bg-surface-hover transition-colors">
                      ...
                      <td className="px-6 py-3 text-sm">
                        ...
                        <span className="text-ink-faint">{"\u2605".repeat(5 - click.rating)}</span>
                      </td>
                      <td className="px-6 py-3 text-sm text-ink-muted max-w-xs truncate">
                        {click.redirected_to}
                      </td>
              ...
              {data.clicks.map((click) => (
                <div key={click.id} className="px-4 py-3">
                  ...
                  <p className="text-xs text-ink-muted mt-0.5">
                    {click.review_requests.clients.name} &middot; {new Date(click.clicked_at).toLocaleDateString()}
                  </p>
                </div>
              ))}
              {data.clicks.length === 0 && (
